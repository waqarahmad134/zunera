...
export default function StaffPage() {
  ...
  const load = useCallback(async () => {
    const body = await api.get<{ orders: Order[] }>("/staff/orders").catch(() => null);
  ...
  async function setStatus(order: Order, status: OrderStatus) {
    ...
    try {
      ...
      setUpdatingId(null);
      await alertDialog(e instanceof ApiError ? e.message : "Could not update status.");
    }
  }
  ...
  async function setPayment(order: Order, paymentStatus: PaymentStatus) {
    ...
    setUpdatingId(order.id);
    try {
      await api.patch(`/staff/orders/${order.id}`, { paymentStatus });
      setUpdatingId(null);
      await load();
    } catch (e) {
      setUpdatingId(null);
      await alertDialog(e instanceof ApiError ? e.message : "Could not update payment.");
    }
  }
