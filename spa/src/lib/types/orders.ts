// Order types, status metadata and formatting helpers.
...
/** Normalizes a timestamp string to something `Date` parses as UTC — API responses are
 * already ISO 8601 (`...Z`), but tolerate a bare "YYYY-MM-DD HH:MM:SS" too. */
function toParsableIso(raw: string): string {
  if (raw.includes("T") || /Z|[+-]\d\d:\d\d$/.test(raw)) return raw;
  return raw.includes(" ") ? `${raw.replace(" ", "T")}Z` : raw;
}

export function formatDate(iso: string): string {
  return new Date(toParsableIso(iso)).toLocaleDateString("en-GB", {
    day: "numeric",
    month: "short",
    year: "numeric",
  });
...
export function formatDateTime(iso: string): string {
  return new Date(toParsableIso(iso)).toLocaleString("en-GB", {
    day: "numeric",
    month: "short",
    hour: "2-digit",
    minute: "2-digit",
