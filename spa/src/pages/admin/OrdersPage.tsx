...
import { formatCurrency, formatDate, type Order, type OrderStatus } from "@/lib/types/orders";
...
export default function OrdersPage() {
  ...
  const load = useCallback(async () => {
    const body = await api.get<{ orders: Order[] }>("/admin/orders").catch(() => null);
    if (body) setOrders(body.orders);
  }, []);
  ...
  async function save() {
    ...
    setMsg(null);
    try {
      await api.patch(`/admin/orders/${openOrder.id}`, {
  ...
  async function remove(o: Order) {
    ...
    try {
      ...
      // Best-effort, same as before — the drawer stays open and the list
      // stays unchanged if this fails.
    }
  }
